-- Villa Rental MVP (safe to run multiple times)
-- Run this if you get "relation already exists" from ADD-VILLA-RENTALS.sql
...
CREATE INDEX IF NOT EXISTS idx_villas_business_id ON villas(business_id);
CREATE INDEX IF NOT EXISTS idx_villa_rentals_villa_id ON villa_rentals(villa_id);
CREATE INDEX IF NOT EXISTS idx_villa_rentals_nfc_code ON villa_rentals(nfc_code);

ALTER TABLE villas ENABLE ROW LEVEL SECURITY;
ALTER TABLE villa_rentals ENABLE ROW LEVEL SECURITY;

-- Drop policies if they exist, then create
DROP POLICY IF EXISTS "Users manage own villas" ON villas;
...
DROP POLICY IF EXISTS "Public read villa rental by nfc" ON villa_rentals;
CREATE POLICY "Public read villa rental by nfc" ON villa_rentals FOR SELECT
  USING (nfc_code IS NOT NULL);

DROP POLICY IF EXISTS "Public read villas for rental" ON villas;
...
DROP TRIGGER IF EXISTS update_villa_rentals_updated_at ON villa_rentals;
